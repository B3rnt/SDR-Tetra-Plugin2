...
namespace SDRSharp.Tetra
{
    unsafe class MmLevel
    {
        ...
        private static int ParseLocationUpdateAcceptExtensions(LogicChannel channelData, int offset, ReceivedData result)
        {
            try
            {
                ...
                // If GI extension not active: don't try to derive GSSI from random bits.
                ...
                // Only after clean terminator: parse 5 + 24 bits GSSI (the one SDRtetra prints)
                ...
                // Find CCK_identifier (64) like SDRtetra prints
                ScanForCck64(channelData, offset, result);

                // Set location update type text: SDRtetra shows "Roaming location updating" for these accepts
                // We'll provide it in logger based on CCK_id==64 && GSSI present (non-ITSI attach).
                return offset;
            }
            catch
            {
        ...
        private static void ScanForCck64(LogicChannel channelData, int offset, ReceivedData result)
        {
            try
            {
                int scanEnd = Math.Min(channelData.Length - 8, offset + 128);
    ...
    internal static unsafe class MmLogger
    {
            ...
            try
            {
                ...
                switch (mmType)
                {
                    ...
                    case MmPduType.D_LOCATION_UPDATE_ACCEPT:
                    {
                        ...
                        // ITSI attach heuristic (SDRtetra example shows no GSSI, but shows ITSI attach)
                        bool isItsiAttach = (cckId == 64 && gssi <= 0);
                        ...
                        // SDRtetra style auth text
                        if (_lastAuthSsi <= 0 || _lastAuthSsi == ssi)
                        {
                            sb.Append(" - ");
                            sb.Append(AuthText());
                        }

                        // Always show CCK_identifier when present
                        if (cckId > 0)
                        {
                        ...
                        {
                            // SDRtetra shows this for your LU accepts
                            if (cckId == 64 && gssi > 0)
                ...
                // (handig) raw blijft, wil je exact SDRtetra -> verwijderen
                sb.Append("  raw=");
        ...
        private static string AuthText()
        {
